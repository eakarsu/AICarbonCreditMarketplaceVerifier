...
// Feature: Agentic verifier reviewing project documents and flagging additionality risks
import React, { useState } from 'react';
import api from '../services/api';

export default function AgenticVerifierReviewingProjectDocumentsAndFPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-agentic-verifier-reviewing-project-documents-and-f/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Agentic verifier reviewing project documents and flagging additionality risks</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-agentic-verifier-reviewing-project-documents-and-f/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
