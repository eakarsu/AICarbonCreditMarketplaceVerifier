...
export default function Only6FrontendPagesVs18BackendRoutesUiGapPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-only-6-frontend-pages-vs-18-backend-routes-ui-gap/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Only 6 frontend pages vs. 18 backend routes — UI gap</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-only-6-frontend-pages-vs-18-backend-routes-ui-gap/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
