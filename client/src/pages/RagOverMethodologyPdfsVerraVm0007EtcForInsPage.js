...
// Feature: RAG over methodology PDFs (Verra VM0007, etc.) for instant protocol Q&A
import React, { useState } from 'react';
import api from '../services/api';

export default function RagOverMethodologyPdfsVerraVm0007EtcForInsPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-rag-over-methodology-pdfs-verra-vm0007-etc-for-ins/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>RAG over methodology PDFs (Verra VM0007, etc.) for instant protocol Q&A</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-rag-over-methodology-pdfs-verra-vm0007-etc-for-ins/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
