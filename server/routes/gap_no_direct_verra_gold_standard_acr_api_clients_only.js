...
// Feature: No direct Verra/Gold Standard/ACR API clients (only generic integrations scaffold)
  ...
  try {
    ...
    if (!process.env.OPENROUTER_API_KEY) {
      return res.status(503).json({
        ...
        feature: 'gap_no_direct_verra_gold_standard_acr_api_clients_only',
      });
    }
    const payload = req.body || {};
    const userPrompt = `Context (JSON):\n${JSON.stringify(payload).slice(0, 4000)}\n\nTask: ${ 'No direct Verra/Gold Standard/ACR API clients (only generic integrations scaffold)' }\n\nReturn JSON: {"summary": string, "actions": [string], "risks": [string], "confidence": number_0_to_1}.`;
    const system = `You are an expert assistant for the feature: 'No direct Verra/Gold Standard/ACR API clients (only generic integrations scaffold)'. Respond with strict JSON.`;
    ...
    const out = { feature: 'gap_no_direct_verra_gold_standard_acr_api_clients_only', model: aiResp?.model, parsed, raw: content };
    if (dbPool) {
      try {
        await dbPool.query(
          'INSERT INTO gap_features (feature_key, user_id, input_payload, output_payload) VALUES ($1, $2, $3, $4)',
          ['gap_no_direct_verra_gold_standard_acr_api_clients_only', req.user?.id || null, JSON.stringify(payload), JSON.stringify(out)]
        );
...
router.get('/health', (req, res) => res.json({ feature: 'gap_no_direct_verra_gold_standard_acr_api_clients_only', ok: true, ai_configured: !!process.env.OPENROUTER_API_KEY }));
...
router.get('/history', async (req, res) => {
  ...
  try {
    ...
    const r = await dbPool.query(
      ...
      ['gap_no_direct_verra_gold_standard_acr_api_clients_only']
    );
